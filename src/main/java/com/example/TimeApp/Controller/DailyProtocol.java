...
import com.example.TimeApp.Entities.SearchDate;
...
public class DailyProtocol {
    ...
    public String addCustomer(SearchDate searchDate) {
        return "Customers/DailyProtocol/searchByDate";
    }

    @PostMapping("/dateSubmit")
    public ModelAndView submitCustomer(@Valid SearchDate searchDate, BindingResult bindingResult, Model model) throws ParseException {
     String[]dates=new String []{searchDate.getStartDate(), searchDate.getEndDate()};
        if (bindingResult.hasErrors() ) {
            model.addAttribute("date", searchDate);
            return new ModelAndView("/Customers/DailyProtocol/searchByDate");
        }
        model.addAttribute("AllProtocols",protocolsService.returnListByDate(dates));
        return new ModelAndView("/Customers/DailyProtocol/myList");
    }
}
